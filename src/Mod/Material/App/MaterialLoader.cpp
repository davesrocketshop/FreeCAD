...
std::shared_ptr<std::list<std::shared_ptr<MaterialLibrary>>> MaterialLoader::getMaterialLibraries()
{
    ...
    // This parameter needs to be used carefully. It is intended to mark the standard materials as ignored
    // when unit testing or when a suitable replacement is provided. Setting it to true could disrupt
    // normal operation of FreeCAD.
